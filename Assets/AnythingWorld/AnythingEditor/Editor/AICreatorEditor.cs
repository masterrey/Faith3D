...
#region Textures
        ...
        private static Texture2D baseTaskExecutedIcon;
        protected static Texture2D BaseTaskExecutedIcon
        {
            get
            {
                if (baseTaskExecutedIcon == null)
                {
                    baseTaskExecutedIcon = Resources.Load("Editor/AICreator/taskExecutedIcon") as Texture2D;
                }
                return baseTaskExecutedIcon;
            }
        }
...
#region Initialization
        ...
        /// </summary>
        [MenuItem("Anything World/AI Creator", false, 23)]
...
#region Resetting Editor Styles
        ...
        {
            AnythingVoice.RequestCommandsFromString(writtenPrompt, EditorNetworkErrorHandler.HandleError);
            allCommandsExecuted = false;
            writtenPrompt = "";
            writtenSubmission = true;
        }

        private void SubmitVoiceCommand()
        {
            AnythingVoice.isRecording = false;
            var clip = AnythingVoice.StopRecording();
            AnythingVoice.ExtractBytesAndProcess(clip, EditorNetworkErrorHandler.HandleError);
            allCommandsExecuted = false;
            ...
            if (AnythingVoice.parsedCommand != null)
            {
                ...
                if (string.IsNullOrEmpty(AnythingVoice.parsedCommand.text))
                {
                    ...
                    for (int i = 0; i < actionCount; i++)
                    {
                        ...
                        var buttonContent = new GUIContent(defaultActions[i].text, defaultActions[i].actionExecuted ? BaseTaskExecutedIcon : null);
        ...
        private Rect DrawCreationCommands(List<Voice.Action> actions, Rect resultsRect, float actionPadding, float actionHeight, float actionWidth, float offset)
        {
            ...
            for (int i = 0; i < actions.Count; i++)
            {
                ...
                var buttonContent = new GUIContent(GetHumanReadableAction(actions[i].actionEnum), actions[i].actionExecuted ? BaseTaskExecutedIcon : null);
            ...
            var allCommandsButtonContent = new GUIContent("Execute All Commands", allCommandsExecuted ? BaseTaskExecutedIcon : null);
