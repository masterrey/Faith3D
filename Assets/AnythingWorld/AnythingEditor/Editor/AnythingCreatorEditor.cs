...
namespace AnythingWorld.Editor
{
    ...
    public class AnythingCreatorEditor : AnythingEditor
    {
        ...
        protected static List<SearchResult> searchResults = null;
        ...
        public static Dictionary<DefaultBehaviourType, BehaviourTree> DefaultBehaviourDictionary
        {
            get
            {
                ...
                if (TransformSettings.NavMeshRiggedAnimalTree != null) temp.Add(DefaultBehaviourType.NavMeshWalkingAnimal, TransformSettings.NavMeshRiggedAnimalTree);
                if (TransformSettings.RiggedAnimalTree != null) temp.Add(DefaultBehaviourType.WalkingAnimal, TransformSettings.RiggedAnimalTree);
                if (TransformSettings.GroundVehicleTree != null) temp.Add(DefaultBehaviourType.WheeledVehicle, TransformSettings.GroundVehicleTree);
                if (TransformSettings.FlyingVehicleTree != null) temp.Add(DefaultBehaviourType.FlyingVehicle, TransformSettings.FlyingVehicleTree);
                if (TransformSettings.StaticBehaviourTree != null) temp.Add(DefaultBehaviourType.Static, TransformSettings.StaticBehaviourTree);
                if (TransformSettings.FlyingAnimalTree != null) temp.Add(DefaultBehaviourType.FlyingAnimal, TransformSettings.FlyingAnimalTree);
                if (TransformSettings.SwimmingAnimalTree != null) temp.Add(DefaultBehaviourType.SwimmingAnimal, TransformSettings.SwimmingAnimalTree);
                return temp;
            }
        }
        #region Filters
        ...
        {
            if (ScriptableObjectExtensions.TryGetInstance<DefaultBehaviourEditorScriptable>(out var behaviourEditor))
            {
                TransformSettings.NavMeshRiggedAnimalTree = behaviourEditor.defaultNavMeshRigged;
                TransformSettings.RiggedAnimalTree = behaviourEditor.defaultRigged;
                TransformSettings.GroundVehicleTree = behaviourEditor.defaultWheeledVehicle;
                TransformSettings.FlyingVehicleTree = behaviourEditor.defaultFlyingVehicle;
                TransformSettings.FlyingAnimalTree = behaviourEditor.defaultFlyingAnimal;
                TransformSettings.SwimmingAnimalTree = behaviourEditor.defaultSwimmingAnimal;
            }
        }
        ...
        protected void MakeResult(SearchResult result, bool forceSerialize = false)
        {
            var inputParams = new List<RequestParameterOption>
            {
                ...
                RequestParameter.IsUsingNavMesh(TransformSettings.UseNavMesh),
            ...
            {
                gameObject = AnythingMaker.MakeByJson(result.data, inputParams.ToArray());
            }
            else
            {
                gameObject = AnythingMaker.MakeById(result.data._id, inputParams.ToArray());
            }
