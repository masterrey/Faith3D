...
namespace AnythingWorld.Editor
{
    public class MyWorldEditor : AnythingCreatorEditor
    {
        ...
        [MenuItem("Anything World/My World", false, 22)]
        ...
        private void InitializeCollectionsAndLikes()
        {
            ...
            if (collections == null || likedObjects == null)
            {
                ...
                if (myWorldTabCategory == MyWorldTabCategory.LISTS && !modelLevel) CollectionProcessor.GetCollections(RefreshCollectionResults, EditorNetworkErrorHandler.HandleError, this);
            }
        }
        ...
        private void DrawMyWorld()
        {
            ...
            if (DrawSquareButton(listsTabRect, new GUIContent("My Lists", myWorldTabCategory == MyWorldTabCategory.LISTS ? StateListsIcon.activeTexture : StateListsIcon.inactiveTexture), myWorldTabCategory == MyWorldTabCategory.LISTS, 14))
            {
                ...
                searchMode = SearchMode.RUNNING;
                CollectionProcessor.GetCollections(RefreshCollectionResults, EditorNetworkErrorHandler.HandleError, this);
            }
        ...
        protected void DrawDeleteCollectionWindow(AnythingEditor window)
        {
            ...
            {
                CollectionProcessor.DeleteCollection(RefreshCollectionResults, currentCollection, EditorNetworkErrorHandler.HandleError, this);
                modelLevel = false;
                currentCollection = null;
                searchMode = SearchMode.RUNNING;
                CollectionProcessor.GetCollections(RefreshCollectionResults, EditorNetworkErrorHandler.HandleError, this);
        ...
        protected void DrawRemoveFromCollectionWindow(AnythingEditor window, SearchResult result)
        {
            ...
            if (GUILayout.Button($"Delete {result.DisplayName} from \"{currentCollection.DisplayName}\"", ButtonInactiveStyle))
            {
                ...
                {
                    CollectionProcessor.RemoveFromCollection(RefreshCollectionResults, result, currentCollection, EditorNetworkErrorHandler.HandleError, this);
                    modelLevel = true;
                    searchMode = SearchMode.RUNNING;
                    CollectionProcessor.GetCollection(RefreshCollectionInternalResults, currentCollection, EditorNetworkErrorHandler.HandleError, this);
                }
                else
                {
                    CollectionProcessor.DeleteCollection(RefreshCollectionResults, currentCollection, EditorNetworkErrorHandler.HandleError, this);
